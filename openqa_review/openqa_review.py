...
class IssueEntry(object):
    ...
    def _format_failure_modules(self, failedmodules):
        return ', '.join(map(lambda m: m['name'], failedmodules))
    ...
    def _format_failure(self, f):
        ...
        if self.args.verbose_test >= 3 and 'prev' in f:
            return '[%s](%s "Failed modules: %s") [(ref)](%s "Previous test")' % (
                f['name'], self._url(f), self._format_failure_modules(f['failedmodules']), self._url(f['prev'])
            )
        elif self.args.verbose_test >= 2:
            return '[%s](%s "Failed modules: %s")' % (f['name'], self._url(f), self._format_failure_modules(f['failedmodules']))
        else:
            return '%s' % f['name']
...
class ArchReport(object):
    ...
    def __init__(self, arch, results, args, root_url, progress_browser, bugzilla_browser):
        ...
        self.issues['new']['todo'].extend(IssueEntry.for_each(self.args, self.root_url,
                                                              filter(lambda r: r['state'] == 'NEW_ISSUE', results_by_bugref.get('TODO', []))))
        self.issues['existing']['todo'].extend(IssueEntry.for_each(self.args, self.root_url,
                                                                   filter(lambda r: r['state'] == 'STILL_FAILING', results_by_bugref.get('TODO', []))))

        if self.args.include_softfails:
            new_soft_fails = filter(lambda r: r['state'] == 'NEW_SOFT_ISSUE', results.values())
            try:
                self.issues['new']['product'].append(IssueEntry(self.args, self.root_url, new_soft_fails, soft=True))
            except ValueError:
                pass
            existing_soft_fails = filter(lambda r: r['state'] == 'STILL_SOFT_FAILING', results.values())
            try:
